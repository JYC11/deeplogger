...
- **Database**: `sqflite` (SQLite) with a single `DatabaseHelper` class + `MigrationRunner` (`lib/database/migration_runner.dart`).
  - **Migrations**: SQL files live in `assets/migrations/`, named `NNN__name.sql` (zero-padded int version), all DDL `IF NOT EXISTS` (idempotent). A `schema_version` meta table records applied versions. To add a migration: drop `002__*.sql`, bump `DatabaseHelper._version`, rerun. `onDowngrade` rejects old>new. Tests inject a disk-based `MigrationRunner` via `DatabaseHelper.useMigrationRunnerForTesting`.
